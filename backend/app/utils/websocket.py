...
def handle_future_completion(future, client_id):
    ...
    try:
        ...
        if no_face_count == 3:
            ...
            # Send anti-spoofing failure notification directly
            if client_id in active_connections:
                websocket = active_connections[client_id]
                try:
                    loop = get_main_loop()
                ...
                logger.warning(f"Client {client_id} not in active connections, cannot send anti-spoofing failure")
        ...
        if processed_users and client_id in active_connections:
            for user in processed_users:
                ...
                # Send notification message directly to client
                if client_id in active_connections:
                    websocket = active_connections[client_id]
                    try:
                        loop = get_main_loop()
        ...
        # Add no face/no matching users notifications if needed
            ...
            if no_face_count > 0:
                ...
                try:
                    ...
                    logger.info(f"✅ Sent no face notification directly to {client_id}")
                except Exception as e:
                ...
                # This prevents interference with already-marked users who should show real-time detection
                try:
                    loop = get_main_loop()
                    asyncio.run_coroutine_threadsafe(_send_message_to_client(websocket, {
                        "client_id": client_id,
                    ...
                    logger.info(f"✅ Sent no matching users notification directly to {client_id}")
                except Exception as e:
        ...
        # Send each real-time detection directly to client instead of queueing
        for detection in real_time_notifications:
            logger.info(f"Sending real-time detection DIRECTLY to client {client_id}: {detection.get('name')} - {detection.get('confidence_str')}")
            if client_id in active_connections:
                websocket = active_connections[client_id]
                try:
